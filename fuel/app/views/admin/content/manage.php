<div class="d-flex justify-content-between align-items-center mb-4">
	...
	<div>
		<a href="<?php echo Uri::base(); ?>admin/users/deleted" class="btn btn-outline-warning me-2">
...
<div class="card mb-4">
	<div class="card-body">
		<form method="GET" action="<?php echo Uri::base(); ?>admin/users" class="row g-3">
			<div class="col-md-4">
			...
			<div class="col-md-3">
				...
				<select class="form-select" id="sort" name="sort">
					...
					<option value="last_login_desc" <?php echo (isset($sort) && $sort === 'last_login_desc') ? 'selected' : ''; ?>>Đăng nhập gần nhất</option>
				</select>
			</div>
					...
					<tbody>
						...
						<tr>
							<td>
								...
								<?php else: ?>
								<input type="checkbox" disabled title="Không thể chọn chính mình">
								<?php endif; ?>
							</td>
							<td>
								<div class="d-flex align-items-center">
							...
							<td>
								<div class="btn-group" role="group">
									...
									<button class="btn btn-sm btn-outline-danger" title="Xóa" 
											onclick="deleteAdmin(<?php echo $user->id; ?>)">
				...
				<nav aria-label="Page navigation" class="mt-4">
					<ul class="pagination justify-content-center">
						...
						<?php if (isset($current_page) && $current_page > 1): ?>
							<li class="page-item">
								<a class="page-link" href="<?php echo Uri::base(); ?>admin/users?page=<?php echo $current_page - 1; ?><?php echo $query_string; ?>">
						...
						<?php for ($i = 1; $i <= $total_pages; $i++): ?>
							...
							<?php else: ?>
								<li class="page-item">
									<a class="page-link" href="<?php echo Uri::base(); ?>admin/users?page=<?php echo $i; ?><?php echo $query_string; ?>"><?php echo $i; ?></a>
								</li>
							<?php endif; ?>
						<?php endfor; ?>
...
function showAlert(type, message) {
	...
	const existingAlerts = document.querySelectorAll('.alert.js-alert');
...
document.addEventListener('DOMContentLoaded', function() {
	...
	bulkDeleteBtn.addEventListener('click', function() {
		...
		if (checkedBoxes.length === 0) {
			showAlert('warning', 'Vui lòng chọn ít nhất một admin để xóa.');
			return;
		}

		if (confirm(`Bạn có chắc chắn muốn xóa ${checkedBoxes.length} admin đã chọn?`)) {
			...
			fetch('<?php echo Uri::base(); ?>admin/users/bulk-delete', {
				method: 'POST',
				headers: {
					'X-Requested-With': 'XMLHttpRequest'
				},
			...
			.catch(error => {
				...
				showAlert('danger', 'Có lỗi xảy ra khi xóa admin.');
			});
		}
	});
});
...
function deleteAdmin(id) {
    ...
    fetch('<?php echo Uri::base(); ?>admin/users/delete/' + id, {
        method: 'POST',
        headers: {
            'X-Requested-With': 'XMLHttpRequest'
        },
