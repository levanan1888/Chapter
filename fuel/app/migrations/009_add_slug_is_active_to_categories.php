...
class Add_slug_is_active_to_categories
{
    public function up()
    {
        // Add slug if missing
        if (!\DBUtil::field_exists('categories', array('slug'))) {
            ...
            // keep slug nullable and no unique index here to avoid seed conflicts
        }
        ...
        if (!\DBUtil::field_exists('categories', array('is_active'))) {
            ...
            try { \DB::query('UPDATE categories SET is_active = 1 WHERE is_active IS NULL')->execute(); } catch (\Exception $e) {}
        }
    }
